...
# def run_shortcut_event(codes):
#     run_shortcut_event_with_sleep(0, codes)
#     return 0

def run_shortcut_event_with_sleep(sleep_time, codes):
    for key, code in codes.items():
        if code != '':
            pre_configured_event(str(code))
            time.sleep(int(sleep_time) / 1000)
    return 0


def pre_configured_event(code):
    win32api.keybd_event(VK_CODE['shift'], 0, 0, 0)
    win32api.keybd_event(VK_CODE['alt'], 0, 0, 0)
    win32api.keybd_event(VK_CODE['ctrl'], 0, 0, 0)
    win32api.keybd_event(VK_CODE[code], 0, 0, 0)
    time.sleep(.05)
    ...
    win32api.keybd_event(VK_CODE[code], 0, win32con.KEYEVENTF_KEYUP, 0)


def clear_string(string=None, *args):
    for i in string:
        if i == ' ':
            event('spacebar')
